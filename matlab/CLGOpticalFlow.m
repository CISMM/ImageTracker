...
for i = 1:iterations
    for y = 2:h-1
        u(y,xx) = (1-relax)*u(y,xx) + relax * (u(y-1,xx) + u(y+1,xx) + u(y,xx-1) + u(y,xx+1) - (Txy(y,xx).*v(y,xx) + Txt(y,xx)))/(4 + Txx(y,xx));
        v(y,xx) = (1-relax)*v(y,xx) + relax * (v(y-1,xx) + v(y+1,xx) + v(y,xx-1) + v(y,xx+1) - (Txy(y,xx).*u(y,xx) + Tyt(y,xx)))/(4 + Tyy(y,xx));
    end;
